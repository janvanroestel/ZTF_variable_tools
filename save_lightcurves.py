...
token = "<token>" # put your Fritz token here. You can get it from your Fritz profile page



def api(method: str,endpoint: str,data: Optional[Mapping] = None,base_url: str = BASE_URL,):
    method = method.upper()
...
def save_newsource(gloria,group_ids,ra,dec,radius=1.5,obj_id=None,
    dryrun=False,period=None):

    # get the lightcurves
    light_curves = get_lightcurves(gloria,ra,dec,2.)
    ...
    if obj_id is None:
        ...
        if not dryrun:
            ...
            response = api(
                "POST", "/api/sources", post_source_data
            )
...
if __name__ == "__main__":
    ...
    # an example source to save
    ra,dec = 97.0823,32.7882
    period = 0.137146333044483
    group_ids = [381,]

    # save source
    save_newsource(gloria,group_ids,ra,dec,period=period)
